...
def solve_tumor_growth(
    ...
    max_drug = np.max(Drug)

    if max_drug > 0:
        normalized_drug = np.mean(Drug) / max_drug
    else:
        normalized_drug = 0.0

    print("\n----- Tumor Model Diagnostics -----")
    print(f"Mean Drug       : {np.mean(Drug):.6f}")
    print(f"Max Drug        : {np.max(Drug):.6f}")
    print(f"Normalized Drug : {normalized_drug:.6f}")
    print("-----------------------------------")
    ...
    for step in range(time_steps):
        ...
        drug_effect = min(drug_effect, 0.50)
        ...
        drug_killing = (
            drug_efficacy * drug_effect * 0.15 * N
        )
