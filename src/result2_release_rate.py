...
def run_release_rate_study(
    ...
    """Run Result 2: drug release rate comparison.

    Parameters
    ----------
    C_np : ndarray
        Final nanoparticle concentration field.

    Returns
    -------
    tuple
        Drug histories, tumor histories, and summary statistics by release case.

    Units
    -----
    Release rate is h^-1. Time is hours. Tumor volume is cell count.
    """

    logger.info("\nRunning Result 2...")
    logger.info("Drug Release Rate Study")
    ...
    for label, rate in release_rates.items():
        ...
        logger.info("t50 = %.2f hours", t50)

        logger.info("Final Release = %.2f%%", final_release)
    ...
    logger.info("\n")
    logger.info("=" * 80)
    logger.info("RESULT 2 VALIDATION SUMMARY")
    logger.info("=" * 80)

    logger.info("\nRelease Constants Used")
    logger.info("----------------------")
    logger.info("Slow   : k = 0.02 h^-1")
    logger.info("Medium : k = 0.05 h^-1")
    logger.info("Fast   : k = 0.10 h^-1")
